import type { SVGProps } from "react";
interface SVGRProps {
  title?: string;
  titleId?: string;
}
const SvgEmojiFaceCelebrationColored = ({
  title,
  titleId,
  ...props
}: SVGProps<SVGSVGElement> & SVGRProps) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    fill="currentColor"
    viewBox="0 0 51 50"
    width="1em"
    height="1em"
    aria-hidden="true"
    focusable="false"
    aria-labelledby={titleId}
    {...props}
  >
    {title ? <title id={titleId}>{title}</title> : null}
    <circle cx={25.578} cy={25} r={19} fill="url(#paint0_radial_8338_30552)" />
    <circle cx={25.578} cy={25} r={19} fill="url(#paint1_radial_8338_30552)" />
    <circle cx={25.578} cy={25} r={17} fill="url(#paint2_linear_8338_30552)" />
    <path fill="url(#pattern0_8338_30552)" d="M3.828 21.5h35V32h-35z" />
    <ellipse
      cx={11.578}
      cy={38.5}
      fill="url(#paint3_radial_8338_30552)"
      rx={5}
      ry={5.5}
    />
    <path fill="url(#pattern1_8338_30552)" d="M5.578 32h11.5v11.5h-11.5z" />
    <ellipse
      cx={5}
      cy={5.5}
      fill="url(#paint4_radial_8338_30552)"
      rx={5}
      ry={5.5}
      transform="matrix(-1 0 0 1 43.578 33)"
    />
    <path
      fill="url(#pattern2_8338_30552)"
      d="M0 0h11.5v11.5H0z"
      transform="matrix(-1 0 0 1 44.578 32)"
    />
    <path
      fill="#6C4325"
      d="M25.645 23.96a1 1 0 0 1 1 1c0 2.544-2.43 4.29-5 4.29s-5-1.746-5-4.29a1 1 0 0 1 2 0c0 1.09 1.151 2.29 3 2.29s3-1.199 3-2.29a1 1 0 0 1 1-1m-14.211-5.802a1 1 0 0 1 1.341-.447l4 2a1.001 1.001 0 0 1 .05 1.762l-3.5 2a1 1 0 0 1-.993-1.736l1.877-1.073-2.328-1.164a1 1 0 0 1-.447-1.342m19.947-.447a1 1 0 0 1 .894 1.789l-2.328 1.164 1.877 1.073a1 1 0 1 1-.992 1.736l-3.5-2a1.001 1.001 0 0 1 .049-1.762z"
    />
    <path
      fill="url(#paint5_linear_8338_30552)"
      d="M7.02 6.883h1.791v2.724H7.02z"
      transform="rotate(-29.523 7.02 6.883)"
    />
    <path
      fill="url(#paint6_linear_8338_30552)"
      d="M41.273 29.926h1.791v2.724h-1.791z"
      transform="rotate(-29.523 41.273 29.926)"
    />
    <path
      fill="url(#paint7_linear_8338_30552)"
      d="M46.171 16.569h1.791v2.724h-1.791z"
      transform="rotate(30 46.17 16.569)"
    />
    <path
      fill="url(#paint8_linear_8338_30552)"
      d="m43.174.162-.826 16.87c-2.224-.086-5.519-1.68-8.398-3.697-2.879-2.016-6.028-4.82-6.347-6.627.009.006 15.571-6.546 15.571-6.546"
    />
    <mask
      id="mask0_8338_30552"
      width={17}
      height={18}
      x={27}
      y={0}
      maskUnits="userSpaceOnUse"
      style={{
        maskType: "alpha",
      }}
    >
      <path
        fill="url(#paint9_linear_8338_30552)"
        d="m43.174.162-.826 16.87c-2.224-.086-5.519-1.68-8.398-3.697-2.879-2.016-6.028-4.82-6.347-6.627.009.006 15.571-6.546 15.571-6.546"
      />
    </mask>
    <g strokeWidth={1.788} mask="url(#mask0_8338_30552)">
      <path
        stroke="url(#paint10_linear_8338_30552)"
        d="M47.265 12.402c-1.69 2.413-6.85 1.716-11.524-1.557C31.066 7.572 28.647 2.962 30.337.55"
      />
      <path
        stroke="url(#paint11_linear_8338_30552)"
        d="M50.117 8.327c-1.69 2.414-6.85 1.717-11.524-1.556S31.5-1.112 33.19-3.526"
      />
      <path
        stroke="url(#paint12_linear_8338_30552)"
        d="M52.654 4.705c-1.69 2.413-6.849 1.716-11.523-1.557-4.675-3.273-7.094-7.883-5.404-10.296"
      />
    </g>
    <path
      fill="url(#paint13_linear_8338_30552)"
      d="M4.961 32.296h1.791v2.724H4.961z"
      transform="rotate(25.453 4.96 32.296)"
    />
    <defs>
      <linearGradient
        id="paint2_linear_8338_30552"
        x1={25.578}
        x2={25.578}
        y1={8}
        y2={42}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#FEE2AD" />
        <stop offset={1} stopColor="#FEE2AD" stopOpacity={0} />
      </linearGradient>
      <linearGradient
        id="paint5_linear_8338_30552"
        x1={7.915}
        x2={7.915}
        y1={6.883}
        y2={9.606}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#8CDBF9" />
        <stop offset={1} stopColor="#468DDC" />
      </linearGradient>
      <linearGradient
        id="paint6_linear_8338_30552"
        x1={42.168}
        x2={42.168}
        y1={29.926}
        y2={32.65}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#8CDBF9" />
        <stop offset={1} stopColor="#468DDC" />
      </linearGradient>
      <linearGradient
        id="paint7_linear_8338_30552"
        x1={47.066}
        x2={47.066}
        y1={16.569}
        y2={19.293}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#FFABAB" />
        <stop offset={1} stopColor="#FF4C4B" />
      </linearGradient>
      <linearGradient
        id="paint8_linear_8338_30552"
        x1={48.977}
        x2={33.19}
        y1={30.474}
        y2={-3.382}
        gradientUnits="userSpaceOnUse"
      >
        <stop offset={0.613} stopColor="#134BFE" />
        <stop offset={0.87} stopColor="#7CC9F5" />
      </linearGradient>
      <linearGradient
        id="paint9_linear_8338_30552"
        x1={48.977}
        x2={33.19}
        y1={30.474}
        y2={-3.382}
        gradientUnits="userSpaceOnUse"
      >
        <stop offset={0.613} stopColor="#134BFE" />
        <stop offset={0.87} stopColor="#7CC9F5" />
      </linearGradient>
      <linearGradient
        id="paint10_linear_8338_30552"
        x1={28.807}
        x2={45.735}
        y1={2.734}
        y2={14.587}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#B5E2FB" />
        <stop offset={1} stopColor="#227CFD" />
      </linearGradient>
      <linearGradient
        id="paint11_linear_8338_30552"
        x1={31.66}
        x2={48.587}
        y1={-1.341}
        y2={10.512}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#B5E2FB" />
        <stop offset={1} stopColor="#227CFD" />
      </linearGradient>
      <linearGradient
        id="paint12_linear_8338_30552"
        x1={34.197}
        x2={51.124}
        y1={-4.963}
        y2={6.889}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#B5E2FB" />
        <stop offset={1} stopColor="#227CFD" />
      </linearGradient>
      <linearGradient
        id="paint13_linear_8338_30552"
        x1={5.119}
        x2={6.735}
        y1={32.487}
        y2={35.028}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#FFE8F2" />
        <stop offset={1} stopColor="#FD6BAD" />
      </linearGradient>
      <radialGradient
        id="paint0_radial_8338_30552"
        cx={0}
        cy={0}
        r={1}
        gradientTransform="rotate(78.559 2.853 16.898)scale(33.9241)"
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#FFEA9F" />
        <stop offset={0.123} stopColor="#FFE380" />
        <stop offset={0.416} stopColor="#FFD250" />
        <stop offset={0.644} stopColor="#FFC122" />
        <stop offset={1} stopColor="#FFB422" />
      </radialGradient>
      <radialGradient
        id="paint1_radial_8338_30552"
        cx={0}
        cy={0}
        r={1}
        gradientTransform="rotate(78.559 2.853 16.898)scale(33.9241)"
        gradientUnits="userSpaceOnUse"
      >
        <stop offset={0.557} stopColor="#E35D12" stopOpacity={0} />
        <stop offset={1} stopColor="#FF24F6" stopOpacity={0.3} />
      </radialGradient>
      <radialGradient
        id="paint3_radial_8338_30552"
        cx={0}
        cy={0}
        r={1}
        gradientTransform="matrix(0 6.5 -5.90909 0 11.578 37.5)"
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#FFD217" />
        <stop offset={1} stopColor="#FBBF56" />
      </radialGradient>
      <radialGradient
        id="paint4_radial_8338_30552"
        cx={0}
        cy={0}
        r={1}
        gradientTransform="matrix(0 6.5 -5.90909 0 5 4.5)"
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#FFD217" />
        <stop offset={1} stopColor="#FBBF56" />
      </radialGradient>
      <pattern
        id="pattern0_8338_30552"
        width={1}
        height={1}
        patternContentUnits="objectBoundingBox"
      >
        <use xlinkHref="#image0_8338_30552" transform="scale(.01429 .04762)" />
      </pattern>
      <pattern
        id="pattern1_8338_30552"
        width={1}
        height={1}
        patternContentUnits="objectBoundingBox"
      >
        <use xlinkHref="#image1_8338_30552" transform="scale(.04348)" />
      </pattern>
      <pattern
        id="pattern2_8338_30552"
        width={1}
        height={1}
        patternContentUnits="objectBoundingBox"
      >
        <use xlinkHref="#image1_8338_30552" transform="scale(.04348)" />
      </pattern>
      <image
        xlinkHref="data:image/png;base64,"
        id="image0_8338_30552"
        width={70}
        height={21}
        preserveAspectRatio="none"
      />
      <image
        xlinkHref="data:image/png;base64,"
        id="image1_8338_30552"
        width={23}
        height={23}
        preserveAspectRatio="none"
      />
    </defs>
  </svg>
);
export default SvgEmojiFaceCelebrationColored;
