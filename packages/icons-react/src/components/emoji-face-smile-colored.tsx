import type { SVGProps } from "react";
interface SVGRProps {
  title?: string;
  titleId?: string;
}
const SvgEmojiFaceSmileColored = ({
  title,
  titleId,
  ...props
}: SVGProps<SVGSVGElement> & SVGRProps) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    width="1em"
    height="1em"
    fill="currentColor"
    viewBox="0 0 51 50"
    aria-hidden="true"
    focusable="false"
    aria-labelledby={titleId}
    {...props}
  >
    {title ? <title id={titleId}>{title}</title> : null}
    <circle cx={25.578} cy={25} r={19} fill="url(#paint0_radial_8338_30553)" />
    <circle cx={25.578} cy={25} r={19} fill="url(#paint1_radial_8338_30553)" />
    <circle cx={25.578} cy={25} r={17} fill="url(#paint2_linear_8338_30553)" />
    <path fill="url(#pattern0_8338_30553)" d="M4.001 14.241h41v12.3h-41z" />
    <path
      fill="#A6321F"
      d="M36.507 20.217c-.007 6.337-2.679 11.446-12.012 11.346S12.5 26.297 12.507 19.96z"
    />
    <mask
      id="mask0_8338_30553"
      width={25}
      height={13}
      x={12}
      y={19}
      maskUnits="userSpaceOnUse"
      style={{
        maskType: "alpha",
      }}
    >
      <path
        fill="#A6321F"
        d="M36.507 20.217c-.007 6.337-2.679 11.446-12.012 11.346S12.5 26.297 12.507 19.96z"
      />
    </mask>
    <g mask="url(#mask0_8338_30553)">
      <path
        fill="url(#paint3_linear_8338_30553)"
        d="M24.496 26.545c-5.789-2.685-10.167 1.262-10.65 1.26.314 1.326 1.038 4.156 1.42 4.865.48.886 2.406 2.223 3.85 3.556s6.77.483 7.253.486c.387.002 4.695-2.915 6.8-4.374.978-1.613 2.74-4.928 1.968-5.286-.965-.448-6.27-3.13-10.64-.507"
      />
      <path
        fill="url(#paint4_linear_8338_30553)"
        d="M9.06 19.296h30.962v4.4H9.06z"
        transform="rotate(.35 9.06 19.296)"
      />
    </g>
    <path
      fill="#6C4325"
      d="M32.527 10.907a1 1 0 1 1 1.298 1.52l-2.456 2.097 2.85 1.33a1 1 0 0 1-.846 1.813l-4.258-1.988a1 1 0 0 1-.226-1.666zm-16.752-.016a1 1 0 0 1 1.412-.082l3.576 3.177a1 1 0 0 1-.26 1.662l-4.296 1.903a1 1 0 0 1-.81-1.83l2.875-1.272-2.413-2.146a1 1 0 0 1-.084-1.412"
    />
    <defs>
      <linearGradient
        id="paint2_linear_8338_30553"
        x1={25.578}
        x2={25.578}
        y1={8}
        y2={42}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#FEE2AD" />
        <stop offset={1} stopColor="#FEE2AD" stopOpacity={0} />
      </linearGradient>
      <linearGradient
        id="paint3_linear_8338_30553"
        x1={24.6}
        x2={24.529}
        y1={25.384}
        y2={36.989}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#F2649C" />
        <stop offset={0.551} stopColor="#F8317E" />
      </linearGradient>
      <linearGradient
        id="paint4_linear_8338_30553"
        x1={24.53}
        x2={24.55}
        y1={19.705}
        y2={23.052}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#E0CBA9" />
        <stop offset={1} stopColor="#fff" />
      </linearGradient>
      <radialGradient
        id="paint0_radial_8338_30553"
        cx={0}
        cy={0}
        r={1}
        gradientTransform="rotate(78.559 2.853 16.898)scale(33.9241)"
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#FFEA9F" />
        <stop offset={0.123} stopColor="#FFE380" />
        <stop offset={0.416} stopColor="#FFD250" />
        <stop offset={0.644} stopColor="#FFC122" />
        <stop offset={1} stopColor="#FFB422" />
      </radialGradient>
      <radialGradient
        id="paint1_radial_8338_30553"
        cx={0}
        cy={0}
        r={1}
        gradientTransform="rotate(78.559 2.853 16.898)scale(33.9241)"
        gradientUnits="userSpaceOnUse"
      >
        <stop offset={0.557} stopColor="#E35D12" stopOpacity={0} />
        <stop offset={1} stopColor="#FF24F6" stopOpacity={0.3} />
      </radialGradient>
      <pattern
        id="pattern0_8338_30553"
        width={1}
        height={1}
        patternContentUnits="objectBoundingBox"
      >
        <use xlinkHref="#image0_8338_30553" transform="scale(.01429 .04762)" />
      </pattern>
      <image
        xlinkHref="data:image/png;base64,"
        id="image0_8338_30553"
        width={70}
        height={21}
        preserveAspectRatio="none"
      />
    </defs>
  </svg>
);
export default SvgEmojiFaceSmileColored;
